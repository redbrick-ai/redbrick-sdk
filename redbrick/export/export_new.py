...
class ExportRedbrick:
    ...
    def export(self, download_data: bool = False, single_json: bool = False, use_name: bool = False) -> None:
        ...
        for dpoint in tqdm(labelsetIter, total=labelsetIter.datapointCount):
            ...
            if single_json:
                ...
                jsonPath = os.path.join(
                    self.target_dir, "{}.json".format(fname)
                )
            ...
            if download_data:
                if not IS_VIDEO:
                    ...
                    dpoint_item = dpoint["items"][0]
                    if use_name:
                        dpoint_name = dpoint["name"]
                    else:
                        dpoint_name = dpoint_item
                    ...
                    while True:
                        ...
                        if file_ext.lower() in [".jpg", ".png", ".bmp"]:
                            #If image name already has an image extension
                            image_filepath = os.path.join(target_data_dir, image_name[:-4] + "({})".format(idx) + image_name[-4:])
                        else:
                            image_filepath = os.path.join(target_data_dir, image_name + "({}).jpg".format(idx))
                        idx += 1
