...
class Datum:
    ...
    def export(self):
        ...
        if self.labelset.task_type == 'BBOX':
            print(colored("[INFO]:", "blue"),
                  "Exporting to %s format" % self.format)
            self.convert_bbox()
    ...
    def cache_bbox(self):
        ...
        if self.labelset.data_type == "IMAGE":
            for i in tqdm(range(len(self.labelset.dp_ids))):
                ...
                image_filepaths.append('obj_train_data/' +
                                       str(dp.image_url_not_signed).replace("/", "_"))
                cv2.imwrite(image_filepath,
                            np.flip(dp.image, axis=2))  # pylint: disable=no-member
                ...
                with open(label_filepath, 'w+') as file:
                    for label in dp.gt._labels:
                        class_idx = taxonomy_mapper[list(
                            label._class.keys())[0]]
        ...
        # VIDEO DATA
            ...
            for i in range(len(self.labelset.dp_ids)):
                print(colored('[INFO]:', 'blue'), 'Exporting %s/%s video(s)' %
                      (i+1, len(self.labelset.dp_ids)))

                dp = self.labelset.__getitem__(i)
                pbar = tqdm(total=len(dp.items))

                # Write labels for each frame here
                temp = {}
                for label in dp.labels:
                    if 'trackid' in label:
                        if label.get("trackid", None) in temp:
                            temp[label["trackid"]].append(label)
                        else:
                            temp[label["trackid"]] = [label]
                    else:
                        raise Exception('No trackid')

                class VideoBBoxLabel:
                    def __init__(self, labelid: str, trackid: str, xnorm:float, ynorm:float, hnorm:float, wnorm:float, frameindex:int, keyframe:bool, end:bool,category:List[List[str]]) -> None:
                    ...
                    def copy(self) -> "VideoBBoxLabel":
                        return VideoBBoxLabel(self.labelid,
                                              self.trackid,
                                              self.xnorm,
                                              self.ynorm,
                                              self.hnorm,
                                              ...
                                              self.category)
                    @classmethod
                    def from_dict(cls, val: Dict)-> "VideoBBoxLabel":
                        return cls(labelid=val["labelid"], 
                                   trackid=val['trackid'],
                                   xnorm=val['bbox2d']['xnorm'], 
                                   ynorm=val['bbox2d']['ynorm'], 
                                   wnorm=val['bbox2d']['wnorm'], 
                                   hnorm=val['bbox2d']['hnorm'],
                                   frameindex=val['frameindex'], 
                                   end=val['end'], 
                                   category=val['category'],
                                   keyframe=val['keyframe'])

                def interpolate(start: VideoBBoxLabel, frameindex: int, end: Optional[VideoBBoxLabel]) -> "VideoBBoxLabel":
                    if not end:
                        result= start.copy()
                ...
                def interpolate2(keyframes: List[Optional[VideoBBoxLabel]], frameindex:int)->"Optional[VideoBBoxLabel]":
                ...
                def expand_list_of_keyframes(keyframes: List[VideoBBoxLabel], num_frames: int)-> List[Optional[VideoBBoxLabel]]:
                ...
                for trackid,labellist in temp.items():
                    ...
                    temp46 = expand_list_of_keyframes(temp45, len(dp.items))
                    output= [None]*len(dp.items)
                ...
                for index in range(len(dp.items)):
                ...
                for idx, item in enumerate(dp.items):
                    pbar.update(1)
                    dp_entry = {}
                    dp_entry["url"] = dp.items_not_signed[idx]
                    dp_entry["labels"] = []

                    # write image data to file
                    image_filepath = os.path.join(self.cache_dir, 'obj_train_data', str(
                        dp.items_not_signed[idx].replace('/', '_')))
                    image_filepaths.append(os.path.join('obj_train_data', str(
                        dp.items_not_signed[idx].replace('/', '_'))))
                    ...
                    with open(label_filepath, 'w+') as file:
                        for label in frame_labels:
                            class_idx = taxonomy_mapper[label.category[0][-1]]
                            file.write('%d %.6f %.6f %.6f %.6f \n' % (
                                class_idx, label.xnorm, label.ynorm, label.wnorm, label.hnorm))
                pbar.close()
    ...
    def cache_video_classification(self):
        ...
        for i in range(len(self.labelset.dp_ids)):
            ...
            while curr_idx != end_idx:
                ...
                if key_index < len(labels) and labels[key_index]['frameindex'] == curr_idx:
                    frame_label = labels[key_index]['category'][0][-1]
                    key_index += 1

                # export the frame to the relvant folder
                if not os.path.isdir(os.path.join(self.cache_dir, video_name, frame_label)):
                    os.makedirs(os.path.join(
                        self.cache_dir, video_name, frame_label))

                frame_data = np.flip(self._url_to_image(
                    dp.items[curr_idx]), axis=2)
                frame_filename = dp.items_not_signed[curr_idx].replace(
                    '/', '_')
                cv2.imwrite(os.path.join(self.cache_dir, video_name,
                                         frame_label, frame_filename), frame_data)
    ...
    def convert_bbox(self):
        ...
        if self.format == 'coco':
            CocoConverter.convert(
                dataset, save_dir=self.export_dir, save_images=True)
            ...
            CvatConverter.convert(
                dataset, save_dir=self.export_dir, save_images=True
            )
        else:
            print(colored(
                "[WARNING]: Invalid format type \'%s\'. Exporting with YOLO format." % self.format))
            YoloConverter.convert(
                dataset, save_dir=self.export_dir, save_images=True)
