"""
Tests for `redbrick.export.public.ExportRepoImpl`.
...
@pytest.mark.unit
def test_datapoints_in_project(export_repo):
    """Test `redbrick.repo.export.Export.datapoints_in_project`"""
    mock_query = Mock(return_value=fixtures.datapoints_in_project_resp)
    with patch.object(export_repo.client, "execute_query", mock_query):
        resp = export_repo.datapoints_in_project(org_id="mock", project_id="mock")
    assert isinstance(resp, int)
    assert resp == 7


@pytest.mark.unit
def test_get_datapoint_latest(export_repo):
    ...
    with patch.object(export_repo.client, "execute_query", mock_query):
        resp = export_repo.get_datapoint_latest(
...
@pytest.mark.unit
def test_task_search(export_repo):
...
@pytest.mark.unit
def test_active_time(export_repo):
