...
def test_task_event_format():
    ...
    assert formatted_task_event["events"][0]["eventType"] == TaskEventTypes.TASK_CREATED.value
    ...
    formatted_task_event = rb_event_utils.task_event_format(task, users)
    assert formatted_task_event["events"][1]["eventType"] == TaskEventTypes.COMMENT_ADDED.value
    assert formatted_task_event["events"][1]["commentId"] == 2
    assert formatted_task_event["events"][1]["commentText"] == "This is a comment on the task."
...
def test_task_event_format_additional_paths():
    ...
    users = {"user1": "John Doe"}
    formatted_task_event = rb_event_utils.task_event_format(task, users)
    assert formatted_task_event["events"][0]["eventType"] == TaskEventTypes.TASK_ACCEPTED.value
    ...
    formatted_task_event = rb_event_utils.task_event_format(task, users)
    assert formatted_task_event["events"][0]["eventType"] == TaskEventTypes.TASK_REJECTED.value
    ...
    formatted_task_event = rb_event_utils.task_event_format(task, users)
    assert formatted_task_event["events"][0]["eventType"] == TaskEventTypes.CONSENSUS_TASK_EDITED.value
    assert formatted_task_event["events"][0]["consensusUsers"][0]["assignee"] == "Jane Smith"
    assert formatted_task_event["events"][0]["consensusUsers"][0]["status"] == "SKIPPED"
    assert formatted_task_event["events"][0]["prevConsensusUsers"][0]["assignee"] == "John Doe"
    assert formatted_task_event["events"][0]["prevConsensusUsers"][0]["status"] == "IN_PROGRESS"
